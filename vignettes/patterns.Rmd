...
log_this <- logger() %>%
  with_receivers(
    ...
    to_text_file("app.log"),         # For local debugging
...
log_this <- logger() %>%
  with_receivers(
    ...
    to_text_file("debug.log"),       # File: everything
...
process_data <- function(data) {
  log_this <- .base_logger %>%
    with_receivers(to_text_file("process_data_detail.log")) %>%
...
log_this <- logger() %>%
  with_receivers(to_text_file("audit.log"))
...
setup_logger_from_config <- function() {
  ...
  for (recv_cfg in cfg$logging$receivers) {
    ...
    if (!is.null(recv_cfg$file)) {
      receivers <- c(receivers, list(to_text_file(recv_cfg$file$path)))
    }
...
log_this <- logger() %>%
  with_receivers(
    to_console(),
    to_json_file("batch_progress.jsonl")
  )
...
sec_logger <- logger() %>%
  with_receivers(
    ...
    to_json_file("security.jsonl", lower = SEC_INFO),
